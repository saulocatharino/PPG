...
videoWidth = frame.shape[1]//2
videoHeight = frame.shape[0]//2
videoChannels = 3
...
bpmTextLocation = (realWidth//2 , realHeight//2)
fontScale = 1
...
while (True):
    ...
    if i > bpmBufferSize:
        cv2.putText(black, "BPM: %d" % bpmBuffer.mean(), bpmTextLocation, font, fontScale, fontColor, lineType)
    else:
        cv2.putText(black, "Calculating BPM...", loadingTextLocation, font, fontScale, fontColor, lineType)

    outputVideoWriter.write(frame)

    if len(sys.argv) != 2:
        cv2.imshow("Monitor", frame)
        cv2.imshow("BPM", black)
